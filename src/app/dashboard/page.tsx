  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Card className="border-border bg-card/90 shadow-xs hover:shadow-md transition">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-foreground">{totalSubs}</div>
        ...
        <Card className="border-border bg-card/90 shadow-xs hover:shadow-md transition">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            ...
            <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-purple-50 text-purple-600 dark:bg-purple-950/80 dark:text-purple-400">
        ...
        <Card className="border-border bg-card/90 shadow-xs hover:shadow-md transition">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Slug: <code className="text-indigo-600 dark:text-indigo-400 font-medium">/c/{church.slug}</code>
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-2 border-border bg-card/90 shadow-xs">
        ...
        <Card className="border-border bg-card/90 shadow-xs flex flex-col justify-between">
          <CardHeader>
            <div className="flex items-center gap-2">
              <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-amber-50 text-amber-600 dark:bg-amber-950/80 dark:text-amber-400">
                <MailIcon className="h-4 w-4" />
              </div>
              <div>
                <CardTitle className="text-base font-bold text-card-foreground">Email Broadcast</CardTitle>
                <CardDescription className="text-xs">Nurture your conference attendees</CardDescription>
              </div>
            </div>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="rounded-xl bg-secondary/60 p-4 border border-border space-y-2">
              <div className="flex items-center justify-between text-xs text-muted-foreground font-medium">
                <span>Total Reachable Audience:</span>
                <span className="font-bold text-foreground">{totalSubs} Subscribers</span>
              </div>
          ...
          <div className="p-6 pt-0">
            <Button asChild className="w-full bg-primary hover:bg-primary/90 text-primary-foreground gap-2 font-semibold shadow-sm">
      ...
      <Card className="border-border bg-card/90 shadow-xs">
        <CardHeader className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <CardTitle className="text-xl font-bold text-card-foreground">Ministry Conferences</CardTitle>
            <CardDescription className="text-xs">
              All live, scheduled, and past online conferences created for your church.
            </CardDescription>
          </div>
          <Button asChild size="sm" className="bg-indigo-600 hover:bg-indigo-700 text-white gap-1.5 font-semibold">
            <Link href="/dashboard/conferences/new">
        ...
        <CardContent>
          {conferences.length === 0 ? (
            <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-border bg-secondary/30 py-12 text-center">
              <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-indigo-50 text-indigo-600 dark:bg-indigo-950/80 dark:text-indigo-400 mb-3">
                <CalendarIcon className="h-6 w-6" />
              </div>
              <h3 className="font-heading text-lg font-bold text-foreground">No conferences yet</h3>
              <p className="text-xs text-muted-foreground max-w-sm mt-1 mb-4">
                Use our AI Generator to create your first online conference page in under 60 seconds.
              </p>
              <Button asChild size="sm" className="bg-indigo-600 hover:bg-indigo-700 text-white font-semibold">
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {conferences.map((conf) => (
                <div
                  key={conf.id}
                  className="flex flex-col justify-between rounded-xl border border-border bg-card p-5 shadow-2xs hover:shadow-md hover:border-primary/40 transition-all space-y-4"
                >
                  <div className="space-y-3">
                    ...
                    <h4 className="font-heading text-base font-bold text-card-foreground line-clamp-1">
                      {conf.title}
                    </h4>
                    ...
                    {conf.speaker_name && (
                      <p className="text-[11px] text-muted-foreground font-medium">
                        🗣️ <span className="text-foreground">{conf.speaker_name}</span>
                      </p>
                    )}
                  </div>

                  <div className="flex items-center gap-2 pt-3 border-t border-border">
                    <Button asChild variant="outline" size="sm" className="flex-1 text-xs font-semibold">
                      <Link href={`/dashboard/conferences/${conf.id}/edit`}>Manage</Link>
                    </Button>
                    <Button asChild variant="ghost" size="sm" className="text-xs font-semibold text-indigo-600 hover:text-indigo-700 dark:text-indigo-400">
